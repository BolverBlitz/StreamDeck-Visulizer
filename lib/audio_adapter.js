    ...
    selectDevice(ctx, canvas, canvasManager) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const out_devices = portAudio.getDevices().filter(device => (device.hostAPIName === "MME" || device.hostAPIName === "Windows WDM-KS") && !device.name.includes('@System32'));
                console.log(portAudio.getDevices())
                const out_devices_name = out_devices.map(device => { if (device.name.length > 42) { return device.name.substring(0, 42) + '...'; } else { return device.name; } });
                ...
                const onEncoderPress = (index) => {
                    if (index === 0) {
                        ...
                        const audioDeviceID = this.#findIdByName(out_devices[resolvedIndex].name, out_devices);

                        // Set the sample rate to the default sample rate of the device
                        const sampleRate = audio.getDefaultSampleRate(out_devices, audioDeviceID);
                        ...
                        process.log.system(`Selected device: ${out_devices[resolvedIndex].name} with ID: ${audioDeviceID}`);
