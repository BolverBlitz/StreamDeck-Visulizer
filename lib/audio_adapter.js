...
class AudioAdapter {
    ...
    #getWaveformColor(bassAmplitude, AmplitudeMultiplier) {
        const exponent = 2.5;

        let normalizedAmplitude = bassAmplitude * AmplitudeMultiplier;
        normalizedAmplitude = Math.min(4000, Math.max(0, normalizedAmplitude));
        let scaledAmplitude = Math.pow(normalizedAmplitude / 4000, exponent);
        let red = 255;
        let green = Math.max(0, 255 - (scaledAmplitude * 255));
        let blue = Math.max(0, 255 - (scaledAmplitude * 255));
        return `rgb(${red}, ${Math.round(green)}, ${Math.round(blue)})`;
    }
    ...
    #drawWaveform(data) {
        ...
        // Set the line color
        this.ctx.strokeStyle = this.#getWaveformColor(data.bassAmplitude, data.AmplituteMultiplayer);
        ...
        for (let i = 0; i < waveform.length; i++) {
            ...
            segmentHeight = segmentHeight << 1; // Bitwise shift to multiply by 2
